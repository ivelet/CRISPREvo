...
class EvoBinaryTokenClassificationModel(nn.Module):
    def __init__(self, config: PeftConfig,
        ...
        self.classifier = nn.Linear(512, 1)  # One binary output per token
        # Due to the BCEWithLogitsLoss, we don't apply the sigmoid function
    ...
    def forward(self, input_ids, attention_mask=None, labels=None):
        ...
        binary_logits = self.classifier(outputs.float())
        ...
        if labels is not None:
            ...
            loss = loss_fct(binary_logits.squeeze(-1), labels.float())

            # Apply attention mask to ignore loss on irrelevant tokens
            masked_loss = loss * valid_mask

            # Compute mean loss only on valid elements
            num_valid = valid_mask.sum()
            if num_valid > 0:
                loss = masked_loss.sum() / num_valid
            else:
                loss = torch.tensor(0.0).to(binary_logits.device)
